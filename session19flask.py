# henry= henry12 ,james = root , mike = mike12 password
#Flask Web App :)
from flask import *
import datetime
...
def fetch_customer_by_vet():
    ...
    return render_template('customer.html', email=session['vet_email'], name=session['vet_name'], documents=documents)
@web_app.route("/fetch-pet/<id>")
...
def delete_pet(id):
    ...
    db.delete(query)
    return render_template('success.html', message="Pet {} deleted".format(pet['name']))
...
def update_pet(id):
    ...
    query = {'_id': ObjectId(id)}
    pet = db.fetch(query)[0]
    return render_template('update-pet.html', email=session['vet_email'],
...
def update_pet_in_db():
    ...
    if len(pet_data_to_update['name']) == 0 or len(pet_data_to_update['breed']) == 0 or pet_data_to_update['age'] <= 0 or len(pet_data_to_update['gender']) == 0:
        return render_template('error.html', message="Name, Breed, Age & Gender cannot be Empty")
    print(pet_data_to_update)
    db = MongoDBHelper(collection="pet")
    query = {'_id': ObjectId(request.form['pid'])}
...
def fetch_all_consultations():
    ...
    print(documents,type(documents))
    # return "Customers Fetched for the Vet {}".format(session['vet_name'])
    return render_template('all-consultations-pet.html',
                           email=session['vet_email'],
                           name=session['vet_name'],
                           documents=documents)
...
def delete_consultation(id):
    ...
    db.delete(query)
    return render_template('success.html', message="Consultation {} deleted".format(consultation['_id']))
@web_app.route("/update-consultations/<id>")
def update_consultations(id):
    db = MongoDBHelper(collection="consultation")
    query = {'_id': ObjectId(id)}
    consultation = db.fetch(query)[0]
...
def update_consultation_in_db():
    ...
    if len(consultation_data_to_update['problem']) == 0 or len(consultation_data_to_update['medicines']) == 0:
        return render_template('error.html', message="Problem and Medicines cannot be Empty")

    print(consultation_data_to_update)
    db = MongoDBHelper(collection="consultation")
    query = {'_id': ObjectId(request.form['cnid'])}
    db.update(consultation_data_to_update, query)
    return render_template('success.html', message="Consultation Problem {} updated successfully ".format(consultation_data_to_update['problem']))
def main():
